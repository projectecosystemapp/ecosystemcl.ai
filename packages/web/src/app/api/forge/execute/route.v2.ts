...
export async function POST(request: Request) {
  try {
    ...
    const { goal, workspaceId, executionMode = 'mcp' } = body;
    ...
    const constraints = await tokenEconomist.analyzeAndConstrain(goal, userId);
    const directive = await tokenEconomist.generateDirective(constraints, goal);
    ...
    const plans = await mcpServer.generatePlans(goal, workspaceState, constraints);
    ...
    return NextResponse.json({
      ...
      websocketUrl: `/api/forge/stream/${storedPlan.id}`,
